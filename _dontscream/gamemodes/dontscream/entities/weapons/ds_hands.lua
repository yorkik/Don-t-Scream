...
if SERVER then
	...
	function SWEP:DrawHUD()
		...
		if self.Owner:InVehicle() then return end
			...
			t.endpos = t.start + self.Owner:GetAimVector() * 75
			t.filter = self.Owner
		...
		if not self:GetFists() then
			...
			if Tr.Hit then
				if self:CanPickup(Tr.Entity) then
					...
					draw.DrawText( LANG.Get('HANDSACT'), "ui.10", Tr.HitPos:ToScreen().x, Tr.HitPos:ToScreen().y + 30, col, TEXT_ALIGN_CENTER )
